...
Webメルカトルの空間参照系IDは "EPSG:3857" です。が、その前には非公式に "EPSG:900913" となっていました。**今も``spatial_ref_sys``にありますが、使ってはいけません**。

## "EPSG:900913" とは

"EPSG:900913" は、Webメルカトルとも言われますが、「WGS84球面メルカトル」とも言えます。WGS84 は、長軸半径 ``6378137``、短軸半径 ``6356752`` の回転楕円体となっています。球面の場合は、短軸半径を長軸半径と 同じ ``6378137`` としています。

ちなみに、900913 は、Google Maps で使用された座標系なので、"google" を "g"→9、"o"→0、"l"→1、"e"(ε)→3 と変換してこうなったようです。シャレですね。

## "EPSG:900913" の変換はおかしい
...
``3857``の結果 (西脇市、日本へそ公園) と ``900913``の結果 (三木市) とが大幅に異なります。Y値が24,000の差 (約20kmの差)となると全く違う投影法です。

## 余談: 約20kmの差について

上で「Y値が24,000の差 (約20kmの差)」と書いていました。「24,000の差」というｙのは``EPSG:3857``ベースでは約24kmの差を指していますが、この数値が本当の距離を示すものではありません。「約20kmの差」は、``900913``への変換の結果を``3857``上にそのまま置いてジオグラフィーにして計算して出したものです。
...
## どうすれば 900913 でも正しく変換できるようになるのでしょう?

なぜこんなことになるのかを見てみよう、``900913`` でも正しく変換できるようにするにはどうすればいいか調べようと思い立ちました。
